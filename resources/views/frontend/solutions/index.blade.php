...
@section('content')
    <section class="resource-archive-hero" data-system-page="solutions">
        @if ($pageBannerUrl)
            <img class="resource-archive-hero__image" data-page-banner-image src="{{ $pageBannerUrl }}"
                alt="{{ $page['title'] }}">
        @endif
                    ...
                    <div class="col-md-6 col-lg-4">
                        <article class="card h-100 overflow-hidden">
                            @if ($solution->image_url)
                                <a href="{{ route('solutions.show', ['solution' => $solution->slug]) }}"><img
                                        class="card-img-top" style="aspect-ratio:16/10;object-fit:cover"
                                        src="{{ $solution->image_url }}" alt="{{ $solution->title }}" loading="lazy"></a>
                            @endif
                            <div class="card-body">
                                <h2 class="h4"><a
                                        href="{{ route('solutions.show', ['solution' => $solution->slug]) }}">{{ $solution->title }}</a>
                                </h2>
                                <p>{{ $solution->excerpt }}</p><a class="section-link"
                                    href="{{ route('solutions.show', ['solution' => $solution->slug]) }}">Xem giải pháp <i
