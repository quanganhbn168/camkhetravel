...
<section class="landing-cta" id="{{ $block['id'] }}" data-landing-block="cta">
    <div class="landing-shell landing-cta__inner">
        <div>
            ...
            <h2>{{ $data['title'] ?? 'Sẵn sàng đồng hành cùng THT Media?' }}</h2>
            @if (filled($data['description'] ?? null))<p>{{ $data['description'] }}</p>@endif
        </div>
        <a class="landing-button landing-button--light" href="{{ $data['cta_url'] ?? '#tu-van' }}" data-landing-event="cta_click" data-block-id="{{ $block['id'] }}">{{ $data['cta_label'] ?? 'Đăng ký ngay' }} <span aria-hidden="true">↗</span></a>
    </div>
</section>
