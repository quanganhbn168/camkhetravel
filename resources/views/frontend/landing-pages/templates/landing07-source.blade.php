...
<div class="landing-page {{ $definition['css_class'] ?? 'landing-page--landing07-source' }} landing-page--source" data-landing-page data-landing-id="{{ $landingPage->id }}" data-track-endpoint="{{ $landingTrackingUrl }}" data-campaign-state="{{ $landingCampaignState }}" data-template-source="landing07-source" style="--landing-primary: {{ $landingTheme['primary'] ?? '#12372a' }}; --landing-accent: {{ $landingTheme['accent'] ?? '#d8a84e' }}; --landing-surface: {{ $landingTheme['surface'] ?? '#f4f7ef' }}; --landing-ink: {{ $landingTheme['ink'] ?? '#14251d' }}">
    ...
    <section class="landing07-source-hero" id="hero">
        @if ($heroMedia?->url)<img class="landing07-source-hero__media" src="{{ $heroMedia->url }}" alt="{{ $heroMedia->alt ?: $heroMedia->title ?: $landingPage->title }}" loading="eager">@endif
        ...
        <div class="landing-shell landing07-source-hero__inner">
            <div class="landing07-source-hero__copy">
                ...
                <h1>{{ $heroData['title'] ?? $landingPage->title }}</h1>
    ...
    @include('frontend.landing-pages.partials.builder-content', ['landingBlocks' => $landingBlocks])
