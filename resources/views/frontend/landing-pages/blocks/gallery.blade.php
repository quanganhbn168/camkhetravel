        ...
        <div class="landing-shell">
            ...
            <div class="landing-gallery__grid">
                @foreach ($block['media_items'] as $media)
                    <a class="glightbox" href="{{ $media->url }}" data-gallery="landing-gallery-{{ $landingPage->id }}" aria-label="Mở ảnh {{ $loop->iteration }}">
                        <img src="{{ $media->url }}" alt="{{ $media->alt ?: $media->title ?: $landingPage->title }}" loading="lazy">
                    </a>
                @endforeach
            </div>
        </div>
