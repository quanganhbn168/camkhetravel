...
@section('content')
    ...
    <article class="bni-article">
        <header class="bni-article__header">@if ($imageUrl)<img src="{{ $imageUrl }}" alt="" aria-hidden="true">@endif<div class="site-shell"><a class="bni-back-link" href="{{ LocalizedUrl::route('bni.articles.index') }}">← Tin tức Lễ chuyển giao</a><p class="bni-experience-kicker">{{ $article->categories->pluck('name')->implode(' · ') ?: ($article->chapter?->short_name ?: 'Tin tức BNI') }}</p><h1>{{ $article->title }}</h1>@if ($article->excerpt)<p>{{ $article->excerpt }}</p>@endif</div></header>
