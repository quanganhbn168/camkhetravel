<div class="position-fixed d-grid site-floating-actions__div-1" aria-label="Liên hệ nhanh">
    @if ($website->hotline)
        <a class="floating-action floating-action--phone" href="tel:{{ preg_replace('/\s+/', '', $website->hotline) }}" aria-label="Gọi {{ $website->hotline }}">
    ...
    @endif
    @if ($website->zalo_url)
        <a class="floating-action floating-action--zalo" href="{{ $website->zalo_url }}" target="_blank" rel="noopener noreferrer" aria-label="Nhắn Zalo">
            <img class="site-floating-actions__media-2" src="{{ asset('images/zalo.svg') }}" alt="">
        </a>
    @endif
</div>
