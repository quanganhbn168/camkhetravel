...
{
    public const ADS = 'landing07_ads';

    public const WEDDING = 'landing07_wedding';

    public const COMMUNICATIONS = 'landing07_communications';

    public const ACADEMY = 'landing07_academy';

    public const ACADEMY_V2 = 'landing07_academy_v2';

    public const OUTSOURCED_MARKETING = 'landing07_outsourced_marketing';

    public const PROFILE = 'landing07_profile';

    public const EVENT_ORGANIZATION = 'landing07_event_organization';

    /** @return array<string, array<string, mixed>> */
    public static function templates(): array
    {
        $templates = [
            self::ADS => self::template(
                'Landing07 / Quảng cáo trực tuyến',
                'Bố cục chuyển thể từ trang dịch vụ quảng cáo: chiến lược, quy trình, phí quản lý và form audit.',
                'landing07-ads',
                '#101828',
                '#f97316',
                '#fff7ed',
                '#111827',
                'Quảng cáo · Audit · Tối ưu · Báo cáo',
                'heroicon-o-chart-bar',
                60,
                'dichvuchayads',
            ),
            self::WEDDING => self::template(
                'Landing07 / Phóng sự cưới',
                'Bố cục hình ảnh cảm xúc cho dịch vụ quay chụp phóng sự cưới toàn quốc và quốc tế.',
                'landing07-wedding',
                '#2f1b2e',
                '#d89a65',
                '#fbf4ef',
                '#261a24',
                'Chụp ảnh · Video · Phóng sự cưới',
                'heroicon-o-heart',
                70,
                'dichvucuoi',
            ),
            self::COMMUNICATIONS => self::template(
                'Landing07 / Giải pháp truyền thông',
                'Bố cục editorial cho chiến lược truyền thông, sản xuất media, quảng cáo và các case study.',
                'landing07-communications',
                '#12372a',
                '#d8a84e',
                '#f4f7ef',
                '#14251d',
                'Chiến lược · Media · Quảng cáo · Case study',
            ...
            self::ACADEMY => self::template(
                ...
                'Bố cục học tập theo hành trình cầm máy, thực hành, hậu kỳ và xây dựng portfolio.',
                'landing07-academy',
                '#202b44',
                '#e1ad5b',
                '#f4f5f8',
                '#172033',
                'Khóa học · Thực hành · Portfolio',
                'heroicon-o-academic-cap',
                90,
                'khoahocnhiepanhthucchien',
            ),
            self::ACADEMY_V2 => self::template(
                'Landing07 / THT Academy Version 2',
                'Bố cục khóa học theo chương trình 12 buổi, workshop, thực hành và đầu ra portfolio.',
                'landing07-academy-v2',
                '#172b35',
                '#e6a84d',
                '#f5f6f1',
                '#132229',
                '12 buổi · 10 tuần · Portfolio',
                'heroicon-o-academic-cap',
                100,
                'khoahocnhiepanhver2',
            ),
            self::OUTSOURCED_MARKETING => self::template(
                'Landing07 / Phòng Marketing thuê ngoài',
                'Bố cục theo hệ thống vận hành marketing: audit, nền tảng, sản xuất, KPI và tối ưu.',
                'landing07-outsourced-marketing',
                '#123c3d',
                '#f2b84b',
                '#eef7f4',
                '#102d2d',
                'Chiến lược · Nội dung · Media · Ads · SEO',
                'heroicon-o-users',
                110,
                'phongmarketingthuengoai',
            ),
            self::PROFILE => self::template(
                'Landing07 / Profile doanh nghiệp',
                'Bố cục hồ sơ năng lực theo mục đích sử dụng, nội dung, hình ảnh, thiết kế và file bàn giao.',
                'landing07-profile',
                '#263b35',
                '#d49c4b',
                '#f5f4ee',
                '#202c28',
                'Nội dung · Hình ảnh · Thiết kế · Hồ sơ năng lực',
                'heroicon-o-document-text',
                120,
                'thietkeprofiledoanhnghiep',
            ),
            self::EVENT_ORGANIZATION => self::template(
                'Landing07 / Tổ chức sự kiện trọn gói',
                'Bố cục theo master plan sự kiện: concept, sản xuất, vận hành và tư liệu sau chương trình.',
                'landing07-event-organization',
                '#182c47',
                '#efb04f',
                '#f2f5f8',
                '#122238',
                'Concept · Kịch bản · Sản xuất · Vận hành',
        ...
        foreach (self::pages() as $page) {
            $templateKey = (string) ($page['template_key'] ?? '');

            if (isset($templates[$templateKey])) {
                $templates[$templateKey]['default_sections'] = $page['sections'];
            }
        }
    ...
    private static function template(
        ...
        return [
            ...
            'settings' => $settings,
            'settings_schema' => [
                'title' => 'Thiết lập '.$label,
                'description' => 'Các trường nhận diện và CTA của template. Nội dung trang, bảng giá và dự án quản lý ở các phần native bên dưới.',
                'icon' => $icon,
                'columns' => 2,
                'fields' => [
                    ['key' => 'landing07_brand_label', 'type' => 'text', 'label' => 'Nhãn thương hiệu', 'max_length' => 120, 'full_width' => true],
                    ['key' => 'landing07_service_line', 'type' => 'text', 'label' => 'Dòng dịch vụ', 'max_length' => 255],
                    ['key' => 'landing07_nav_cta', 'type' => 'text', 'label' => 'Nhãn CTA đầu trang', 'max_length' => 100],
                    ['key' => 'landing07_footer_text', 'type' => 'textarea', 'label' => 'Dòng chân trang', 'max_length' => 255, 'rows' => 2, 'full_width' => true],
                ],
            ],
            'default_settings' => $settings,
            'default_sections' => [],
            'source_name' => 'Landing07 source snapshot · Laravel catalog',
            'source_path' => 'landing-07/'.$sourceDirectory.'/data.php',
            'icon' => $icon,
            'is_active' => true,
